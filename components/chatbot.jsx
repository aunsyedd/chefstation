  ...
  return (
    <>
        ...
        >
              ...
              <div className="flex gap-2 self-start max-w-[85%]">
                ...
                <div className="bg-white dark:bg-zinc-700 border border-zinc-200 dark:border-zinc-600 px-4 py-3 rounded-xl rounded-tl-sm flex gap-1 items-center">
                  {[0, 1, 2].map((i) => (
                    <span
                      key={i}
                      className="w-1.5 h-1.5 bg-zinc-400 rounded-full inline-block animate-bounce"
          ...
          <div className="flex flex-wrap gap-1.5 px-3 py-2 border-t border-zinc-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800 max-h-[72px] overflow-y-auto flex-shrink-0">
            {QUICK_CHIPS.map((c) => (
              <button
                ...
                className="text-xs px-2.5 py-1 rounded-full border border-orange-400 text-orange-600 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors whitespace-nowrap"
              >
                {c.label}
              </button>
            ))}
          ...
          <div className="flex gap-2 px-3 py-2.5 border-t border-zinc-200 dark:border-zinc-700 bg-white dark:bg-zinc-900 flex-shrink-0">
            ...
            <button
              ...
              className="w-8 h-8 rounded-full text-white flex items-center justify-center flex-shrink-0 text-sm disabled:opacity-50 transition-opacity"
              style={{ background: "#B85C38" }}
            >
              ➤
            </button>
      ...
      <button
        ...
        className="fixed bottom-6 right-6 w-14 h-14 rounded-full text-white flex items-center justify-center z-50 shadow-lg transition-transform hover:scale-105 active:scale-95 text-2xl"
        style={{ background: "#B85C38" }}
